...
def test(epoch, save_mode_path):
    ...
    AUROCs, Accus, Senss, Specs,_,_ = epochVal_metrics_test(model, test_dataloader, thresh=0.4)  
...
if __name__ == '__main__':
     ...
     anchor_bank = init_semantic_anchors(args.num_classes, anchor_dim, device='cuda')
     ...
     for com_round in range(args.rounds):
          ...
          if com_round*args.local_ep > 20:
               ...
               for idx in unsupervised_user_id :
                    ...
                    local = trainer_locals[idx]
                    optimizer = optim_locals[idx]
                    w, loss, op = local.train(args, net_locals[idx], optimizer, com_round*args.local_ep, anchor_bank.detach())
                    w_locals[idx] = copy.deepcopy(w)
                    optim_locals[idx] = copy.deepcopy(op)
                    loss_locals.append(copy.deepcopy(loss))
